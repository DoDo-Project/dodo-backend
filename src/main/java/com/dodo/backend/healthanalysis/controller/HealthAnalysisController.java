...
public class HealthAnalysisController {
     ...
     * 건강 분석 결과를 삭제합니다.
     *
     * @param analysisId 삭제할 분석 보고서 ID
     * @param userDetails 인증된 사용자 정보
     * @return 삭제 결과 응답
     */
    @Operation(summary = "건강 분석 결과 삭제", description = "analysisId 기준으로 건강 분석 결과를 삭제합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공적으로 삭제되었습니다.",
                    content = @Content(schema = @Schema(implementation = AnalysisDeleteResponse.class))),
            @ApiResponse(responseCode = "400", description = "잘못된 요청입니다.",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = @ExampleObject(name = "400 Bad Request", value = "{\"status\": 400, \"message\": \"잘못된 요청입니다.\"}"))),
    ...
    public ResponseEntity<AnalysisDeleteResponse> deleteAnalysis(
        ...
        UUID userId = UUID.fromString(userDetails.getUsername());
        log.info("건강 분석 삭제 요청 - User: {}, AnalysisId: {}", userId, analysisId);
        return ResponseEntity.ok(healthAnalysisService.deleteAnalysis(userId, analysisId));
    }
}
